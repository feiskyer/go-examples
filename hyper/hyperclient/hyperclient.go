...
func testService() {
	...
	svcList, err := hyperClient.ListServices(podId)
	if err != nil {
		fmt.Printf("List service error: %v\n", err)
	} else {
		fmt.Printf("Service list: %v\n", svcList)
	}

	newServices := HyperService{
		ServiceIP:   "10.255.3.7",
		ServicePort: 1234,
		Hosts: []HyperServiceBackend{
			{
				HostPort: 4321,
				HostIP:   "10.10.10.10",
			},
	...
	err = hyperClient.UpdateServices(podId, []HyperService{newServices})
	if err != nil {
		fmt.Println("Update service failed: ", err)
	}
...
func GetPodinfo(podID string) (*PodInfo, error) {
	hyperClient := NewHyperClient()

	values := url.Values{}
	values.Set(KEY_POD_NAME, podID)
	body, _, err := hyperClient.call("GET", "/pod/info?"+values.Encode(), "", nil)
	if err != nil {
		return nil, err
	}
...
func main() {
	fmt.Println(GetPodinfo("pod-zpIOTSAjmM"))
}
